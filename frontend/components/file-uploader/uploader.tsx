...
export function Uploader({ value, onChange, fileTypeAccepted }: iAppProps) {
  ...
  async function handleRemoveFile() {
    // 1) ne double-clique pas
    if (fileState.isDeleting) return;

    // 2) ⬅️ AJOUT ICI : pas de key en storage → on nettoie juste l’UI et on sort
    if (!fileState.key) {
      if (fileState.objectUrl && !fileState.objectUrl.startsWith("http")) {
        URL.revokeObjectURL(fileState.objectUrl);
      }
    ...
    // 3) sinon on appelle l’API pour supprimer en storage
    try {
      setFileState((prev) => ({ ...prev, isDeleting: true }));

      const response = await fetch("/api/s3/delete", {
        method: "POST", // ou DELETE si ta route le gère
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        ...
        toast.error(err?.error ?? `Failed to remove file (${response.status})`);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: false,
    maxSize:
      fileTypeAccepted === "image" ? 5 * 1024 * 1024 : 5000 * 1024 * 1024, // 5mo calculation
